...
public interface Model {
   ...
   * Thmodel.getActivePuzzle().getHeight() an IndexOutOfBoundsException if r or c is out of bounds,
   * or an IllegalArgumentException if the cell is not type CORRIDOR
   */
  void addLamp(int r, int c);

  /**
   * Removes a lamp if one exists from the active puzzle at the cell at row r, column c.
   * Thmodel.getActivePuzzle().getHeight() an IndexOutOfBoundsException if r or c is out of bounds,
   * or an IllegalArgumentException if the cell is not type CORRIDOR
   */
   ...
   * clue). If the cell itself contains a lamp, this method should also return true.
   * Thmodel.getActivePuzzle().getHeight() an IndexOutOfBoundsException if r or c is out of bounds,
   * or an IllegalArgumentException if the cell is not type CORRIDOR
   */
   ...
   * lamp. Thmodel.getActivePuzzle().getHeight() an IndexOutOfBoundsException if r or c is out of
   * bounds, or an IllegalArgumentException if the cell is not type CORRIDOR
   */
  boolean isLamp(int r, int c);

  /**
   * Returns true only if, in the active puzzle, the cell at row r, column c contains a user-placed
   * lamp that is in direct view of another lamp (i.e. the lamp is illegally placed).
   * Thmodel.getActivePuzzle().getHeight() an IndexOutOfBoundsException if r or c is out of bounds,
   * or an IllegalArgumentException if the cell does not currently contain a lamp
   */
